...
void Background::AddBGPart(const wchar_t* filename, float scale, const Vector2& rate, int priority)
{
	m_parts.emplace_back(&m_fileManager,filename, scale, rate, priority);
}

void Background::Update()
{
	m_scrollX = m_camera.GetRange().Left();
	m_scrollY = m_camera.GetRange().Bottom();
}
...
void Background::BackDraw()
{
	...
	for (auto& bgpart : m_parts)
	{
		bgpart.Draw(m_camera);
	}
